...
Use `TIAE_SKIP_PREFLIGHT=1` only after the same check has completed separately.
...
CUDA_VISIBLE_DEVICES=0,1,2,3 bash scripts/train_multitask.sh \
  ...
  outputs/tiae_multitask_goal_s3072_e5
```
...
`-- checkpoints/
    ...
    `-- tiae_multitask_goal_*/   # task shards created by shared training
```
