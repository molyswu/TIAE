#!/usr/bin/env python3
"""Validate a TIAE checkout before training or evaluation starts."""
...
class Report:
    ...
    def payload(self) -> dict[str, Any]:
        ...
        return {
            "schema_version": "tiae-preflight-v1",
            "mode": self.mode,
    ...
    def print(self) -> None:
        print(f"TIAE preflight: {self.mode}\n")
...
def check_config_contract(report: Report, mode: str) -> None:
    try:
        ...
        return
    cfg = OmegaConf.load(REPO / "config/train/tiae_goal.yaml")
    model = OmegaConf.load(REPO / "config/train/model/tiae.yaml")
    expected = {
    ...
    variant_expected = {
        "lewm": (0.0, 0.0, None),
        "tiae_inverse": (0.1, 0.0, "four_slot"),
        "tiae_goal_only": (0.0, 0.05, "four_slot"),
        "tiae_goal": (0.1, 0.05, "four_slot"),
        "tiae_waypoint": (0.1, 0.05, "four_slot"),
    }
    variant_errors = []
