...
case "$VARIANT" in
  lewm) CONFIG=lewm ;;
  inverse) CONFIG=tiae_inverse ;;
  goal_only) CONFIG=tiae_goal_only ;;
  displacement|goal) CONFIG=tiae_goal; VARIANT=displacement ;;
  waypoint) CONFIG=tiae_waypoint ;;
  *) echo "variant must be lewm, inverse, goal_only, displacement, or waypoint" >&2; exit 2 ;;
esac

if [[ "${TIAE_SKIP_PREFLIGHT:-0}" != "1" ]]; then
