"""TIAE: Topological Intent-Action Embedding framework.

Wraps the JEPA world model with topology-preserving learning: the structure of
relationships among intents is preserved in the action space, rather than any
particular intent being mapped to any particular action.

Reference: Wu, Wu & Wu, "From Intent Alignment to Intent Topology:
A Topological Theory of Goal-Directed Embodied Control", 2026.
"""
...
class TIAE(nn.Module):
    """Topological Intent-Action Embedding — a JEPA-based world model that learns
    a topology-preserving map from the intent manifold to the action manifold.
    ...
    Architecture:
        ...
        └───────────┬─────────────────────┬───────────────┘
                    │                     │
          ┌─────────▼────────┐  ┌─────────▼──────────────┐
          │ Forward Predictor │  │  Intent-Action Actor    │
          │   (JEPA head)     │  │  Dual-Channel Encoding  │
          │  z_t → z_{t+1}   │  │  Intent → Action Law    │
          └──────────────────┘  └─────────────────────────┘
                    │                     │
          ┌─────────▼─────────────────────▼──────────────┐
          │            Topology-Preserving Loss           │
          │  d_intent(i,j) ≈ d_action(μ_i, μ_j)          │
          └──────────────────────────────────────────────┘
    """
    ...
    def __init__(
        ...
        # ── TIAE-specific parameters ──
        dual_channel: DualChannelEncoder | None = None,
        topo_loss_weight: float = 0.05,
        topo_distance_metric: str = "cosine",
        topo_subset_size: int = 256,
    ) -> None:
        super().__init__()
        if intent_mode not in self.VALID_INTENT_MODES:
            raise ValueError(
                f"intent_mode must be one of {sorted(self.VALID_INTENT_MODES)}, "
                f"got {intent_mode}"
            )
        ...
        self.topo_loss = TopologyPreservingLoss(
            distance_metric=topo_distance_metric,
            subset_size=topo_subset_size,
        )
        self.topo_loss_weight = float(topo_loss_weight)
        self.dim_estimator = IntrinsicDimensionEstimator(k=5)

    # ── Forward pass (encoding / predicting) ──
    ...
    # ── TIAE: topology-preserving intent-action mapping ──
    ...
    def compute_topo_loss(
        ...
        """Compute the topology-preserving loss between intent and action manifolds.

        For a batch of (z_t, intent_t, a_t) triplets, this loss ensures that the
        pairwise distance structure among intents is approximately preserved among
        the induced action means.
        """
        if self.intent_actor is None or self.topo_loss_weight <= 0:
            return {"topo_loss": embeddings.new_zeros(())}

        T = embeddings.size(1)
        current = embeddings[:, intent_start: T - 1]
        goal = embeddings[:, -1:].detach().expand_as(current)

        if self.intent_mode == "goal_displacement":
            intent = goal - current
        elif self.intent_mode == "waypoint":
            remaining = torch.arange(
                current.size(1), 0, -1,
                device=embeddings.device, dtype=embeddings.dtype,
            ).view(1, -1, 1)
            intent = (goal - current) / remaining
        else:
            raise ValueError(f"Unknown intent mode: {self.intent_mode}")

        B, T_seq, D = current.shape
        z_flat = current.reshape(B * T_seq, D)
        intent_flat = intent.reshape(B * T_seq, D)

        prev_act_flat = previous_actions[:, intent_start: T - 1]
        prev_act_flat = prev_act_flat.reshape(B * T_seq, -1)
        prev_act_emb = self.action_encoder(prev_act_flat.unsqueeze(1)).squeeze(1)

        if self.dual_channel is not None:
            fused = self.dual_channel(z_flat, intent_flat)
            # Use fused features as the joint representation for topology loss
            action_mean = self.intent_actor.action_mean(
                fused, intent_flat, prev_act_emb
            )
            topo_loss = self.topo_loss(intent_flat, action_mean)
        else:
            action_mean = self.intent_actor.action_mean(
                z_flat, intent_flat, prev_act_emb
            )
            topo_loss = self.topo_loss(intent_flat, action_mean)

        return {
            "topo_loss": self.topo_loss_weight * topo_loss,
            "topo_loss_raw": topo_loss.detach(),
        }

    # ── Intent / action interface (shared with JEPA) ──
    ...
    def rollout(
        ...
        """Roll out action candidates for optional CEM checks."""
...
def create_tiae_from_jepa(
    ...
    *,
    ...
    dual_channel_hidden_dim: int = 256,
    dual_channel_num_heads: int = 4,
) -> TIAE:
    """Wrap an existing JEPA model into a TIAE model with topology loss.

    Args:
        jepa_model: existing JEPA / TIAE model instance
        topo_loss_weight: weight for topology-preserving loss
        topo_distance_metric: distance metric for topology loss
        topo_subset_size: random subset size for efficient topology loss
        use_dual_channel: whether to add dual-channel encoding
        dual_channel_hidden_dim: hidden dim for dual-channel encoder
        dual_channel_num_heads: attention heads for dual-channel encoder

    Returns:
        TIAE model instance
    """
    dual_channel = None
    if use_dual_channel and jepa_model.intent_actor is not None:
        dual_channel = DualChannelEncoder(
            state_dim=jepa_model.intent_actor.embed_dim,
            intent_dim=jepa_model.intent_actor.embed_dim,
            hidden_dim=dual_channel_hidden_dim,
            num_heads=dual_channel_num_heads,
        )

    tiae = TIAE(
        encoder=jepa_model.encoder,
        predictor=jepa_model.predictor,
        action_encoder=jepa_model.action_encoder,
        projector=jepa_model.projector,
        pred_proj=jepa_model.pred_proj,
        intent_actor=jepa_model.intent_actor,
        intent_mode=jepa_model.intent_mode,
        predict_residual=jepa_model.predict_residual,
        actor_warmstart=jepa_model.actor_warmstart,
        dual_channel=dual_channel,
        topo_loss_weight=topo_loss_weight,
        topo_distance_metric=topo_distance_metric,
        topo_subset_size=topo_subset_size,
    )
    return tiae
