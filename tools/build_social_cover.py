#!/usr/bin/env python3
"""Render native 4K and 2K social covers for TIAE."""
...
def render_cover(repo: Path) -> Image.Image:
    ...
    logo = fit(Image.open(assets / "tiae-wordmark-light.png"), 1780, 610)
    canvas.alpha_composite(logo, (1920 - logo.width // 2, 690 - logo.height // 2))

    draw = ImageDraw.Draw(canvas)
    draw_text(draw, (1920, 1180), "TIAE", 84, WHITE, "bold")
    draw.line((1290, 1258, 2550, 1258), fill=BLUE_BRIGHT, width=5)
    draw_text(draw, (1920, 1344), "Topological Intent-Action Embedding for Search-Free World Models", 55, "#EEF4FC")
    ...
    draw_text(ImageDraw.Draw(canvas), (1920, 2110), "molyswu.github.io/TIAE", 24, "#8396B1", "medium")
...
def main() -> None:
    ...
    cover.save(assets / "tiae-social-cover-4k.png", optimize=True)
    cover.resize((2560, 1440), Image.Resampling.LANCZOS).save(
        assets / "tiae-social-cover-2k.jpg",
        quality=96,
        subsampling=0,
        optimize=True,
    )
