#!/usr/bin/env python3
"""Build the cinematic TIAE website film from audited local artifacts.
...
TIAE_SR = (85.78, 100.0, 97.67, 97.89)
GUARDED_SR = (92.22, 99.78, 97.44, 98.0)
LEWM_MACRO = 85.75
TIAE_MACRO = 95.33
GUARDED_MACRO = 96.86

PROJECT_URL = "https://molyswu.github.io/TIAE/"
GITHUB_URL = "https://github.com/molyswu/TIAE"
...
class RolloutPair:
    ...
    def at(self, progress: float) -> tuple[Image.Image, Image.Image, Image.Image]:
        ...
        tiae_index = min(len(self.tiae) - 1, int(clamp(progress) * (len(self.tiae) - 1)))
        shared_start = crop_reference(self.tiae[0])
        rollout_mix = smooth(progress / 0.14)
        lewm = Image.blend(shared_start, crop_agent(self.lewm[index]), rollout_mix)
        tiae = Image.blend(shared_start, crop_agent(self.tiae[tiae_index]), rollout_mix)
        return lewm, tiae, crop_goal(self.tiae[0])
...
def scene_demos(t: float, pairs: list[RolloutPair]) -> Image.Image:
    ...
    for pair, (x, y), accent in zip(pairs, locations, TASK_COLORS):
        lewm, tiae, goal = pair.at(progress)
        ...
        tiae_box = (x + 500, video_y, x + 830, video_y + 330)
        paste_cover(frame, lewm, lewm_box)
        paste_cover(frame, tiae, tiae_box)
        draw = ImageDraw.Draw(frame)
        draw.rectangle(lewm_box, outline="#6F7C90", width=3)
        draw.rectangle(tiae_box, outline=CORAL, width=4)
        ...
        label_bar(tiae_box, "TIAE · DIRECT", CORAL)
...
def draw_tiae_demo_card(frame: Image.Image, box, current: Image.Image) -> None:
    ...
    text(draw, ((x0 + x1) / 2, y0 + 23), "TIAE · DIRECT", 17, CORAL, "bold", "mm")
...
def scene_figure1(t: float, pairs: list[RolloutPair], include_operator: bool = True) -> Image.Image:
    ...
    for pair, sample, start, target, accent in zip(pairs, samples, starts, targets, TASK_COLORS):
        ...
        draw_tiae_demo_card(single_frame, box, sample[0])
...
def scene_isomorphism(t: float, pairs: list[RolloutPair]) -> Image.Image:
    ...
    if phase_alpha > 0:
        ...
        text(draw, (grammar_x + 290, 438), "SAME TIAE INPUT GRAMMAR", 20, "#215E8E", "bold", "mm")
...
def scene_bars(t: float) -> Image.Image:
    ...
    group_w = (x1 - x0) / 4
    for index, (task, baseline, direct, guarded) in enumerate(zip(TASKS, LEWM_SR, TIAE_SR, GUARDED_SR)):
    ...
    legend_y = 972
    legends = ((180, "#8390A2", "LeWM · CEM 300×30"), (535, BLUE, "TIAE · Direct · 0"), (860, CORAL, "TIAE · Guarded A · 384"))
...
def scene_headlines(
    ...
    def paint_red(layer_draw: ImageDraw.ImageDraw, _overlay: Image.Image) -> None:
        ...
        text(layer_draw, (right_x, latency_y + 22), "TIAE Direct", 16, CORAL, "medium", "mm")
...
def build_share_film(
    ...
    hero = assets / "tiae-hero-film.mp4"
    if not hero.exists():
    ...
    share_cut = 60.3
    destination = assets / "tiae-project-film-share.mp4"
    subprocess.run(
        [
            "ffmpeg",
            "-y",
...
def load_rollouts(experiment: Path) -> list[RolloutPair]:
    lewm_root = experiment / "recovery_TIAE_eval_fleet/results/eval_staging/lewm_s42/epoch_5/cache"
    tiae_root = experiment / "recovery_TIAE_eval_fleet_delta/results/eval_staging/delta_condition_full_s3072/epoch_5/cache"
    pairs = []
    for task, key in zip(TASKS, TASK_KEYS):
        episode = EPISODES[key]
        lewm = read_video(lewm_root / f"recovery_lewm_{key}_s42/env_{episode}.mp4")
        tiae = read_video(tiae_root / f"recovery_delta_full_{key}_s3072/env_{episode}.mp4")
        pairs.append(RolloutPair(task, key, episode, lewm, tiae))
    return pairs
...
def build_film(
    ...
    logo = film_logo(Image.open(assets / "tiae-wordmark-light.png").convert("RGBA"))
    ...
    poster.save(assets / "tiae-hero-film-poster.jpg", quality=96, subsampling=0, optimize=True)
    if preview_only:
    ...
    write_video(
        assets / "tiae-hero-film.mp4",
        (render(index / FPS) for index in range(round(total * FPS))),
    )
    print(f"Built {total:.1f}s TIAE film at {WIDTH}×{HEIGHT}, {FPS} fps")
...
def main() -> None:
    ...
    correlation = json.loads((assets / "goal-tiae-alignment.json").read_text())["points"]
    build_film(
        assets,
        pairs,
        points,
