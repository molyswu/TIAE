"""Train the paper-aligned, single-task TIAE model.

TIAE: Topological Intent-Action Embedding — a JEPA-based world model that
learns a topology-preserving map from the intent manifold to the action
manifold for search-free goal-directed control.
"""
...
def tiae_forward(self, batch, stage, cfg):
    """TIAE forward pass with JEPA prediction, intent-action NLL, and topology loss."""
    ...
    # ── TIAE topology-preserving loss ──
    topo_weight = float(
        getattr(cfg.loss, "topo_weight", 0.0)
        if hasattr(cfg.loss, "topo_weight") else OmegaConf.select(cfg, "loss.topo_weight", default=0.0)
    )
    if topo_weight > 0 and isinstance(self.model, TIAE):
        self.model.topo_loss_weight = topo_weight
        topo = self.model.compute_topo_loss(
            embeddings,
            batch["action"],
            batch["previous_action"],
            intent_start=int(cfg.loss.intent.goal_start),
        )
    else:
        topo = {"topo_loss": embeddings.new_zeros(())}
    ...
    metrics = {
        ...
        **intent,
        "topo_loss": topo.get("topo_loss", embeddings.new_zeros(())),
    }
    self.log_dict(
...
@hydra.main(
    version_base=None, config_path="./config/train", config_name="tiae_goal"
)
def run(cfg):
    ...
    module = spt.Module(
        ...
        forward=partial(tiae_forward, cfg=cfg),
        optim=optimizers,
    )
        ...
        json.dumps(
            {
                ...
                "loss_weights": {
                    ...
                    "topo": float(getattr(cfg.loss, "topo_weight", 0.0)),
                },
