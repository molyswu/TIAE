#!/usr/bin/env python3
"""Train TIAE with a shared encoder/projector and task-specific small heads.

TIAE: Topological Intent-Action Embedding — learns distance-preserving maps
from intent manifold to action manifold for search-free goal-directed control.
"""
...
from train import build_dataset, configure_deterministic_math, tiae_forward
...
class LossHarness:
    """Minimal interface expected by ``tiae_forward`` outside Lightning."""
...
def configure_task(config_dir: Path, task: str, args):
    ...
    with open_dict(cfg):
        ...
        cfg.loss.topo_weight = variant["topo_weight"]
...
def save_task_checkpoint(model, cfg, task: str, epoch: int, args, metrics):
    ...
    atomic_json(
        checkpoint_dir / f"training_metadata_epoch_{epoch}.json",
        {
            ...
            "loss_weights": {
                "forward": 1.0,
                "local": VARIANTS[args.variant]["local_weight"],
                "goal": VARIANTS[args.variant]["goal_weight"],
                "topo": VARIANTS[args.variant]["topo_weight"],
                "sigreg": 0.03,
            },
            "window_protocol": {
                "local_start": 0,
...
def parse_args():
    ...
    parser.add_argument("--output-prefix", default="tiae_multitask_goal")
    ...
    if rank == 0:
        atomic_json(
            args.run_dir / "run_identity.json",
            {
                ...
                "architecture": "shared encoder/projector; task-specific forward/action heads; topology-preserving loss",
                "tasks": identities,
                "seed": args.seed,
                "epochs": args.epochs,
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for step in range(total_steps):
            ...
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                output = tiae_forward(harness, batch, "train", cfg)
